...
def main():
    ...
    # Запускаем сортировку прямыми вставками:
    ar1 = copy.deepcopy(ar)
    t = time.time()
    insertionSort(ar1)
    t = (time.time() - t) * 1000
    print("Длительность сортировки прямыми вставками: {:.0f} мс".format(t))
    print(ar1)  # Выводим на экран

    # Запускаем сортировку прямыми вставками с бисекцией:
    ar1 = copy.deepcopy(ar)
    t = time.time()
    insertionSortBisection(ar1)  # insertionSortBisection
    t = (time.time() - t) * 1000
