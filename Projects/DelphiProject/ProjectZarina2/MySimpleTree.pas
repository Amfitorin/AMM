...
// 31-1) ОСНОВНАЯ Функция определения МИНИМАЛЬНОЙ длины пути, проходящего через заданный узел
function MinLenWay(v: tree_ptr): Integer;
// 31-2) ВСПОМОГАТЕЛЬНАЯ Функция определения МИНИМАЛЬНОЙ длины пути, проходящего через заданный узел
function MinLenWayOne(v: tree_ptr): Integer;

// 30-1) Основная Функция (по обеим ветвям) вычисления суммы ВСЕХ элементов пути минимальной длины, проходящей через заданный узел
function SumAllKey_MinLenWay(v: tree_ptr): Integer;
// 29-1) Вспомогательная Функция (по одной ветви) вычисления суммы ВСЕХ элементов пути МИНИМАЛЬНОЙ длины, проходящей через заданный узел
function SumAllKey(v: tree_ptr): Integer;

// 32-1) Основная функция поиска узла, через который проходит путь МИНИМАЛЬНОЙ длины
function FindNodeBetween_MinLenWay(v: tree_ptr): tree_ptr;
// 33-1) Вспомогательная процедура обхода дерева и поиска узла, через который проходит путь МИНИМАЛЬНОЙ длины
